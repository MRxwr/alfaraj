...
if ( isset($_FILES["image"]) )
{
	...
	for ( $i = 0 ; $i < sizeof($_FILES["image"]["tmp_name"]) ; $i++ ){
		...
		$image 	 = $filenewname;
		$sql = "INSERT INTO `news_gallery`
				(`image`, `details`, `detailsAr`)
				VALUES 
				('".$image."', '".$details."', '".$detailsAr."')
				";
...
if ( isset($_GET["delId"]) ){
	$sql = "DELETE FROM `news_gallery` WHERE `id` LIKE '".$_GET["delId"]."'";
    $result = $dbconnect->query($sql);
}
?>
<body>
...
<input type="text" name="detailsAr" class="form-control" required>
</div>
</div>
